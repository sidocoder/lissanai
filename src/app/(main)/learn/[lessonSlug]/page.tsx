...
const AccordionItem = ({ topic, isOpen, onToggle }: AccordionItemProps) => (
  <div>
    ...
    >
      ...
      <FiChevronDown
        className={`text-gray-600 w-6 h-6 flex-shrink-0 transition-transform duration-300 ${
          isOpen ? "rotate-180" : ""
        }`}
      />
    </button>
...
export default function LessonDetailPage() {
  ...
  const lesson = learnPageLessons.find((l) => slugify(l.title) === lessonSlug);
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 text-center">
          ...
          <p className="mt-4">
            We could not find the lesson you were looking for.
          </p>
          <Link
            href="/learn"
            className="mt-6 inline-block bg-[#337fa1] text-white font-semibold py-2 px-4 rounded-lg hover:bg-[#3e5b69]"
          >
            Back to All Lessons
          </Link>
        </main>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="relative bg-blue-100/50 border border-blue-200 rounded-2xl p-6 pr-28 sm:pr-32 mb-12 shadow-sm">
          <p className="text-gray-700 max-w-lg">
            Welcome to the {pageTitle} section! Here you will learn essential
            concepts to improve your English communication skills.
          </p>
          ...
          <div className="space-y-4">
            {lesson.subTopics.map((topic) => (
              <AccordionItem
                ...
                onToggle={() =>
                  setOpenAccordion(
                    openAccordion === topic.title ? null : topic.title
                  )
                }
              />
            ))}
